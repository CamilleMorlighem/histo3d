...
"""
This script enables convex decomposition of concave polygons. 
It is used by the script plots_to_footprints.py in the reconstruction of 
individual building footprints from building plots. 

Implementation based on Schteppe's poly-decomp.js.
Algorithms based on Mark Bayazit's Poly Decomp.

Credit: https://github.com/wsilva32/poly_decomp.py
"""
...
def polygonGetCutEdges(polygon):
    ...
    for i in range(0, len(polygon)):
        if polygonIsReflex(polygon, i):
            print(polygonIsReflex(polygon,i))
            for j in range(0, len(polygon)):
                ...
                if polygonCanSee(polygon, i, j):
                    ...
                    print(polygonCanSee(polygon, i, j))
...
def polygonSlice(polygon, cutEdges):
    ...
    if isinstance(cutEdges, list) and len(cutEdges) != 0 and isinstance(cutEdges[0], list) and len(cutEdges[0]) == 2 and isinstance(cutEdges[0][0], list):
        ...
        print(polygon)
        print(cutEdge)
        x,y=shapely.geometry.Polygon(polygon).exterior.xy
        plt.plot(x,y)
        x,y=shapely.geometry.LineString(cutEdge).xy
        plt.plot(x,y)
        plt.show()
